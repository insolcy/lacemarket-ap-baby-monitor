...
只有商品卡片带有 Lace Market 官方 `New` 标签、详情分类属于 Dresses、且卖家位于美国时才发送邮件；Relist、非 Dresses 分类和非美国卖家商品都会被记录为已处理，但不会发送邮件，之后也不会重复提醒。邮件包含商品首图、名称、品牌、价格、状态、卖家地区和可点击链接；点击图片也会打开商品页。监控使用按创建时间倒序的水位线算法，旧商品翻页或重新排序不会被当作新品。
